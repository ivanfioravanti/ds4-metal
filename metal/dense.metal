...
// Three token rows share a weight load, retaining each scalar reduction.
template<bool ROUND_BF16>
void kernel_mul_mv_q8_0_f32_token_triple_impl(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device char * dst,
    ...
    const short NSG = FC_mul_mv_nsg;
    const int r0 = tgpig.x * NR0;
    const int token = tgpig.y * 3;
    const int second = min(token + 1, args.ne11 - 1);
    const int third = min(token + 2, args.ne11 - 1);
    device const float * y0 = (device const float *)(src1 + (uint64_t)token * args.nb11);
    device const float * y1 = (device const float *)(src1 + (uint64_t)second * args.nb11);
    device const float * y2 = (device const float *)(src1 + (uint64_t)third * args.nb11);
    device const block_q8_0 * ax[NR0];
    FOR_UNROLL (short row = 0; row < NR0; ++row)
        ax[row] = (device const block_q8_0 *)(src0 + (uint64_t)(r0 + row) * args.nb01);
    float sum0[NR0] = {0.f}, sum1[NR0] = {0.f}, sum2[NR0] = {0.f};
    const short ix = tiisg / (N_SIMDWIDTH / NQ);
    const short il = tiisg % (N_SIMDWIDTH / NQ);
    for (int ib = sgitg * NQ + ix; ib < args.ne00 / QK8_0; ib += NSG * NQ) {
        float yl0[NQ], yl1[NQ], yl2[NQ];
        FOR_UNROLL (short i = 0; i < NQ; ++i) {
            yl0[i] = y0[ib * QK8_0 + il * NQ + i];
            yl1[i] = y1[ib * QK8_0 + il * NQ + i];
            yl2[i] = y2[ib * QK8_0 + il * NQ + i];
        }
        FOR_UNROLL (short row = 0; row < NR0; ++row) {
            device const int8_t * qs = ax[row][ib].qs + il * NQ;
    ...
    if (token + 1 < args.ne11)
        helper_mv_reduce_and_write<NR0, ROUND_BF16>((device float *)dst + (uint64_t)(token + 1) * args.ne0,
            sum1, r0, args.ne01, tiisg, sgitg, shmem);
    threadgroup_barrier(mem_flags::mem_threadgroup);
    if (token + 2 < args.ne11)
        helper_mv_reduce_and_write<NR0, ROUND_BF16>((device float *)dst + (uint64_t)(token + 2) * args.ne0,
            sum2, r0, args.ne01, tiisg, sgitg, shmem);
}

kernel void kernel_mul_mv_q8_0_f32_token_triple(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device char * dst,
        ...
        ushort tiisg [[thread_index_in_simdgroup]],
        ushort sgitg [[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_q8_0_f32_token_triple_impl<false>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}

kernel void kernel_mul_mv_q8_0_f32_token_triple_bf16(
        constant ds4_metal_args_mul_mv & args,
        device const char * src0,
        device const char * src1,
        device char * dst,
        ...
        ushort tiisg [[thread_index_in_simdgroup]],
        ushort sgitg [[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_q8_0_f32_token_triple_impl<true>(args, src0, src1, dst, shmem, tgpig, tiisg, sgitg);
}
